...
Alternative: OpenAI GPT-4 or hybrid approach
## Cloud Services & Storage
MongoDB Atlas (free tier for start)
AWS S3 or Cloudinary for file storage (videos, PDFs, images)
...
- Stay on topic related to the learning material
...
try{
const message =awaitthis.client.messages.create({
model:'claude-sonnet-4-20250514',
max_tokens:1024,
messages:[
## {
role:'user',
content:this.buildSummarizePrompt(content, summaryType)
## }
## ],
## });
...
## {
role:'user',
content:this.buildQuizPrompt(content, questionCount, difficulty, questi
## }
## ],
## });
const quizText = message.content[0].text;
// Parse JSON response from AI

const quizData =this.parseQuizJSON(quizText);
return quizData;
## }catch(error){
...
model:'claude-sonnet-4-20250514',
max_tokens:1024,
system:this.buildTutorSystemPrompt(context),
...
## }
## }
asyncgenerateLearningPath(userProgress, preferences){
try{
const message =awaitthis.client.messages.create({
model:'claude-sonnet-4-20250514',
max_tokens:1024,
...
## {
role:'user',
content:this.buildLearningPathPrompt(userProgress, preferences)
## }
## ],
## });
return message.content[0].text;
...
thrownewError('Failed to generate learning path');
## }
## }
// Prompt builders
...
- ${summaryType ==='bullet-points'?'Format as bullet points with • markers':'Wr
- Highlight important terms and definitions
- Target length: ${lengthGuide[summaryType]}
Provide only the summary, no preamble:`;
## }
buildQuizPrompt(content, questionCount, difficulty, questionTypes){
return`You are an expert educational quiz creator. Generate exactly ${questionC
## Content:
## ${content}
## Requirements:
...
- For true-false: provide a statement and correct answer
- Ensure questions cover different aspects of the content
- Make questions practical and application-focused when possible
Return ONLY a valid JSON object with this exact structure (no markdown, no additiona
